...
def render(name,items,base=''):
 ...
 for a in items:
  ...
  rows.append(f'<item><title>{escape(a["title"] or "")}</title><link>{escape(a["url"])}</link><guid isPermaLink="true">{escape(a["url"])}</guid><pubDate>{date}</pubDate><description>{escape(a["title"] or "")}</description><content:encoded><![CDATA[{a["content_html"] or ""}]]></content:encoded></item>')
